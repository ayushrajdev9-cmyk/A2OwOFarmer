...
A2_CRED_FILE = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'a2_cred.json')
LOGIN_ATTEMPTS = {}
BLOCK_DURATION = 300  
MAX_ATTEMPTS = 5

def load_auth_config():
    # Try a2_cred.json first (standalone mode)
    if os.path.exists(A2_CRED_FILE):
        try:
            with open(A2_CRED_FILE, 'r') as f:
                cfg = json.load(f)
            if cfg.get('secret_key') == "generate_a_random_long_secret_key_here_please":
                new_secret = secrets.token_hex(32)
                cfg['secret_key'] = new_secret
                with open(A2_CRED_FILE, 'w') as f:
                    json.dump(cfg, f, indent=4)
            return cfg
        except:
            pass
    # Fallback to auth.json in config dir
...
if auth_cfg:
    app.secret_key = auth_cfg.get('secret_key', 'a2_fallback_secret')
else:
...
def test_security():
    ...
    if sec:
        ...
        sec._show_desktop_notification("Test: A2 Security Alert working!")
