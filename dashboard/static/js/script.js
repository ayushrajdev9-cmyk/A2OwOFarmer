...
function showToast(message, type = 'success') {
    const toast = document.getElementById('a2-toast');
    const msgEl = document.getElementById('toast-message');
    if (!toast || !msgEl) return;

    msgEl.innerText = message;
    toast.className = `a2-toast show ${type}`;
...
function createModuleCard(title, data, path) {
    ...
    card.innerHTML = `
        <div class="module-header" onclick="toggleDropdown(this, event)">
            ...
            <span class="icon-svg" style="--icon: url('/static/assets/a2_icons/chevron-down.svg');"></span>
        </div>
            ...
            h += `
                <div class="gem-tier-item ${key}">
                    ...
                    <div class="module-toggle ${val ? 'on' : 'off'}" onclick="toggleMod('${fullPath}', this, event)">
                        <span class="icon-svg" style="--icon: url('/static/assets/a2_icons/toggle-${val ? 'on' : 'off'}.svg');"></span> ${val ? 'ON' : 'OFF'}
                    </div>
                </div>
            `;
...
function renderField(path, f, v) {
    ...
    if (f.type === 'toggle') return `<div class="field-group"><label class="field-label">${f.l}</label><div class="module-toggle ${v ? 'on' : 'off'}" onclick="toggleMod('${path}', this, event)"><span class="icon-svg" style="--icon: url('/static/assets/a2_icons/toggle-${v ? 'on' : 'off'}.svg');"></span> ${v ? 'ON' : 'OFF'}</div></div>`;
    ...
    return `
        <div class="field-group">
            ...
            <div class="stepper-container">
                <button class="stepper-btn" onclick="updateStepper('${path}', -1, event)"><span class="icon-svg" style="--icon: url('/static/assets/a2_icons/minus.svg');"></span></button>
                ...
                <button class="stepper-btn" onclick="updateStepper('${path}', 1, event)"><span class="icon-svg" style="--icon: url('/static/assets/a2_icons/plus.svg');"></span></button>
            </div>
        </div>
    `;
}
...
function toggleMod(p, el, ev) {
    ...
    el.innerHTML = `<span class="icon-svg" style="--icon: url('/static/assets/a2_icons/toggle-${v ? 'on' : 'off'}.svg');"></span> ${v ? 'ON' : 'OFF'}`;
    checkDirty();
}
                ...
                <div class="sec-account-card ${d.status === "PAUSED" ? 'alert-active' : ''} ${isActive ? 'selected' : ''}" onclick="selectAccount('${acc.id}')">
                    <div class="sec-acc-header">
                        <div class="sec-acc-info">
                            ${acc.avatar ? `<img src="${acc.avatar}" class="account-avatar-lg">` : '<span class="icon-svg" style="--icon: url(\'/static/assets/a2_icons/discord.svg\');"></span>'}
                    ...
                    <div class="sec-acc-stats">
                        <div class="sec-mini-stat">
                            <span class="icon-svg" style="--icon: url('/static/assets/a2_icons/check-to-slot.svg'); background-color: var(--success);"></span>
                            <div class="val">${d.security.captchas}</div>
                            <div class="lbl">Solved</div>
                        </div>
                        <div class="sec-mini-stat">
                            <span class="icon-svg" style="--icon: url('/static/assets/a2_icons/user-slash.svg'); background-color: var(--danger);"></span>
                            <div class="val">${d.security.bans}</div>
                            <div class="lbl">Bans</div>
                        </div>
                        <div class="sec-mini-stat">
                            <span class="icon-svg" style="--icon: url('/static/assets/a2_icons/warning.svg'); background-color: var(--warning);"></span>
...
function renderScheduler(states) {
    ...
    list.innerHTML = items.map(item => {
        ...
        if (item.in_queue) {
            statusHtml = `<span style="color:var(--success); font-size:0.8rem; font-weight:bold;"><span class="icon-svg" style="--icon: url('/static/assets/a2_icons/sync.svg'); animation: spin 2s linear infinite;"></span> QUEUED</span>`;
            progress = 100;
        } else {
...
function renderQuests(quests, timer) {
    ...
    if (timer) {
        timerEl.innerHTML = `<span class="icon-svg" style="--icon: url('/static/assets/a2_icons/clock.svg'); width: 14px; height: 14px;"></span> Next quest in: ${timer}`;
